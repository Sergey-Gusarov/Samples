...
public class Replicator {
	  ...
	  public void FullReplicate(IConverterService converterService) throws AuthFaliException, AsyncResultException, ConnectionException{
		 ...
		 //List<String> ids =storyEntities.stream().map(x->x.getStoryId()).collect(Collectors.toList());
			
	 	//Конвертация в SObject 
	 	List<SObject> upsertedSFEntities = storyEntities.stream().map(x->converterService.ConvertToSF(x)).collect(Collectors.toList()); 
	      	
	      	//При этом обновляются объекты с присутствующими идентификаторами SalesForce
		 	//а вставляются с отсутствующими.
			List<UpsertResult> updatedResults = getPartnerConnectionUtils().UpsertToSF(upsertedSFEntities, converterService.getStoryIdNameInSF());

			//Извлекаем идентификаторы для вставленных записей и составляем список ошибок
			//При сопоставлении идентификаторов расчет (надежда) идет на то что список возвращенных записей соотвествует списку отправленных в методе UpsertToSF
			//других вариантов (кроме как запрашивать отдельным запросом) для сопоставления нет.
	    	...
	    	int updated = 0;
	      	...
	      	updatedResults = getPartnerConnectionUtils().UpsertToSF(failedSFEntities, converterService.getStoryIdNameInSF());
	      	// getConnection().upsert("BF_Visits_StoryCLM_Id__c", failedVisits.toArray(new SObject[0]));
	    	...
	    	//Выберем все элементы из SF, чтобы проверить наличие каждого в Story
	    	
	    	String sfQueryAll = MessageFormat.format("SELECT Id, {0} FROM {1}", converterService.getStoryIdNameInSF(), converterService.getSFTable());
	    	...
	    	for(int i=0;i<sobjects.size();i+=5){
	    		...
	    		String[] storyIdsRange = sobjRange.stream().map(x->(String)x.getField(converterService.getStoryIdNameInSF())).toArray(String[]::new);
	    		List<String> storyRange = storyService.Find(storyIdsRange).GetResult().stream().map(x->x.getStoryId()).collect(Collectors.toList());
	    		List<String> sfdeletingIds =  sobjRange.stream().filter(sf-> !storyRange.contains(sf.getField(converterService.getStoryIdNameInSF()))).map(x->x.getId()).collect(Collectors.toList());
	  ...
	  {
		  //Сразу пробуем получяем сервис табличного лога, чтобы туда все ошибки срать
		  StoryCLMTableService<StoryLog> slogService = getStoryConnector().GetTableService(StoryLog.class, converterService.getLogTableId());
		  Date thisReplicationDate = new Date();
		  StoryLog slog = new StoryLog();
	      slog.Date = thisReplicationDate;
		  ...
		  logger.info("Start replicate for StoryType: " +  converterService.getStoryType().getSimpleName());
	    	...
	    	List<IStoryEntity> upsertedStoryEntities = new ArrayList<IStoryEntity>();
	      	for(int ii=0;ii<ups_ids.size();ii+=5){
	      		upsertedStoryEntities.addAll(storyService.Find(ups_ids.subList(ii, Math.min(ii+5, ups_ids.size())).toArray(new String[0])).GetResult());
	      	}
	    
	      	//Конвертация в список SObject
	      	List<SObject> upsertedSFEntities = upsertedStoryEntities.stream().map(x->converterService.ConvertToSF(x)).collect(Collectors.toList());
    		...
    		{
    		 	String query = MessageFormat.format("SELECT Id FROM {0} where "+ converterService.getStoryIdNameInSF()+ " in ({0}) ",converterService.getSFTable(), Utils.JoinByComma(removeIdsPortion));
		    	logger.info("SOQL: " + query);
		    	getPartnerConnectionUtils().QueryPartList(
							query, 
							resultPortion-> {
								getPartnerConnectionUtils().DeleteFromSF(resultPortion.stream().map(so->{return so.getId();}));
							});
    		});
	    		...
	    		Integer sfcount = (Integer) getPartnerConnectionUtils().QueryAll("Select Count(id) cnt from "+ converterService.getSFTable()).get(0).getField("cnt");
